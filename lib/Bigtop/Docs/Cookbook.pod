...
For quick syntax reference consult L<Bigtop::Docs::Keywords> for more
complete information consult L<Bigtop::Docs::Syntax> or the
source code of C<Bigtop::Keywords> from which tentmaker draws its
...
something else use a colon as for C<birth_day> in the child table.  Indicate
optional fields with a leading plus sign.  Specify literal defaults with
an equal sign as for anniversary C<preferred_gift>.
...
app directly from an existing postgres 8 database, see
C<Bigtop::Docs::QuickStart>.
...
You don't have to list column information on the command line, if you
don't bigtop will create your app with default columns you probably won't
like.  To avoid that, try using tentmaker:
...
Note that tentmaker also understands column specifications, so feel free
to include those when starting it.  But, it allows you to edit many things
that you can't specify at the command line.  For example, tentmaker
lets you specify a data validation constraint for each column.  There
are many other things it can help with.
